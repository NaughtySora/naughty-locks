...
class Semaphore {
  ...
  constructor(shared, { offset = 0, concurrency } = {}) {
    const size = Semaphore.#size;
    if (shared.byteLength < size) {
      throw new Error(`SharedArrayBuffer should be at least ${size} bytes`);
    }
    const counter = this.#counter = new Int32Array(shared, offset, CHANNEL);
    this.#offset = offset;
...
class ExclusiveSemaphore {
  ...
  constructor(shared, { offset = 0, concurrency } = {}) {
    ...
    const counter = this.#counter = new Int32Array(shared, offset, 2);
